...
protected:
  ...
  virtual void OpenTrace() {
    if (!vcd_file_name_.empty() && vcd_trace_ == NULL) {
      ...
      vcd_trace_->set_time_resolution("0.1 us");
...
int main(int argc, char **argv) {
  ...
  TestBench<Vslip_tx_rx_tb> *tb = new TestBench<Vslip_tx_rx_tb>;

  tb->SetTraceFile("slip_tx_rx_tb-verilator.vcd");
  tb->Initialize();
